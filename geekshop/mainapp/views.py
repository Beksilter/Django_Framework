...
def products(request, id_category=None, page=1):
    ...
    products = Product.objects.filter(category_id=id_category).select_related('category').order_by("price") if id_category \
        else Product.objects.all().select_related('category').order_by("price")
    paginator = Paginator(products, per_page=3)
    try:
        products_paginator = paginator.page(page)
    except PageNotAnInteger:
